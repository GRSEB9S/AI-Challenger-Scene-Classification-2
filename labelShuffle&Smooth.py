...
if __name__ == "__main__":
    ...
    val_data = AIC_scene(
        ...
        Transform=transforms.Compose([
            AIC_scene_data.Scale(224),
            AIC_scene_data.TenCrop(224),
            AIC_scene_data.ToTensor(eval=True),
            AIC_scene_data.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225]) # ImageNet
        ]))
    val_loader = DataLoader(val_data,batch_size=args.batchSize,shuffle=True,num_workers=args.workers)
    ...
    # ----- label smoothing
    utility_Func.confusionMatrix(os.path.join(args.path,"ai_challenger_scene_train_20170904","shuffle_label.txt"),
